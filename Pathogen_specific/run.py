...
pp_anchor = anchors[["uniprot_id", "pathway_id"]].rename(columns={"pathway_id": "map_pathway_id"})
...
pd_infer = drug_infer[["pathway_id", "drug_id"]].rename(columns={"pathway_id": "map_pathway_id"})
...
protein_pathway["protein_idx"] = protein_pathway["uniprot_id"].map({p: i for i, p in enumerate(protein_ids)})
protein_pathway.dropna(inplace=True)

protein_pathway["pathway_idx"], pathway_ids = encode(protein_pathway["map_pathway_id"])

pathway_drug["pathway_idx"] = pathway_drug["map_pathway_id"].map({p: i for i, p in enumerate(pathway_ids)})
...
# ==================================================
# LOAD DRUG LOOKUP CSV
# ==================================================
drug_lookup = pd.read_csv("data/drug_lookup.csv")

# Build drug_idx → drug_name mapping
drug_name_map = {}
for idx, drug_id in drug_map.items():
    ...
    if len(row) > 0:
        ...
        drug_name_map[idx] = drug_id  # fallback: show ID
...
pp_edge = torch.tensor(proteins[["pathogen_idx", "protein_idx"]].values.T, dtype=torch.long)
pw_edge = torch.tensor(protein_pathway[["protein_idx", "pathway_idx"]].values.T, dtype=torch.long)
pd_edge = torch.tensor(pathway_drug[["pathway_idx", "drug_idx"]].values.T, dtype=torch.long)
...
def predict(pathogen, top_k=5):
    ...
    scores = torch.matmul(emb["drug"], emb["pathogen"][pid])

    vals, idx = torch.topk(scores, top_k)

    return [
        (drug_name_map[i], float(v))
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if q_clean not in pathogen_rev:
            disease_match = proteins[proteins["disease_name"].str.lower() == q]["pathogen"].unique()
